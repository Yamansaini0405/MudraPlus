      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div className="flex flex-col md:flex-row justify-between items-start mb-16 gap-8" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div className="max-w-xl">
            ...
            <h2 className="text-4xl md:text-5xl font-semibold text-gray-900 leading-tight">
              The <span className="text-[#133E5A]">MudraPlus</span> Difference
            </h2>
          ...
          <motion.button className="bg-[#F18810] hover:bg-blue-600 transition-colors text-white px-8 py-4 rounded-full font-medium text-sm uppercase tracking-wider shadow-lg" initial={{ scale: 0.98, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.06 }}>
            Learn More
          </motion.button>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-4 flex justify-center lg:justify-start">
            <motion.div className="bg-[#133E5A] text-white p-10 rounded-xl w-full max-w-xs shadow-2xl relative overflow-hidden" initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <div className="relative z-10">
                ...
                <div className="mt-8 flex gap-2">
                   <div className="w-10 h-10 rounded-full bg-[#F18810] flex items-center justify-center">
                      <ArrowUpRight size={20} className="text-[#133E5A]" />
                   </div>
                </div>
              </div>
              
              {/* Decorative Circle in Card */}
              <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-[#F18810] rounded-full opacity-90"></div>
            </motion.div>
          </div>
            ...
            {benefits.map((benefit, idx) => (
              <motion.div
                ...
                className={`group flex items-center justify-between p-6 rounded-lg transition-all cursor-pointer ${
                  benefit.active 
                  ? 'bg-[#133E5A] text-white shadow-xl translate-x-2' 
                  : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                }`}
              >
                <div className="flex items-center gap-6">
                  <span className={`text-lg font-bold ${benefit.active ? 'text-white/80' : 'text-[#133E5A]'}`}>
                    {benefit.id}
                  </span>
                ...
                <ArrowUpRight 
                  size={24} 
                  className={`${benefit.active ? 'text-white' : 'text-gray-400 group-hover:text-[#133E5A]'} transition-colors`} 
                />
              </motion.div>
            ))}
          </div>
