    ...
    <div className="min-h-screen bg-[#ffffff] py-15 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full border border-blue-200 text-[#0D4715] font-medium text-sm mb-4">
            Our Services
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">
            Loans we Offer to <span className="text-[#0D4715]">Help You</span>
          </h2>
        </div>
              ...
              <button
                ...
                className={`w-full h-25 flex items-center gap-4 p-5 rounded-xl transition-all duration-300 border text-lg font-semibold ${
                  activeTab.id === service.id 
                    ? "bg-[#0D4715] text-white border-[#0D4715] text-xl shadow-lg shadow-blue-200 scale-[1.02]" 
                    : "bg-white text-slate-700 border-slate-100 hover:border-blue-300"
                }`}
              >
                <span className={activeTab.id === service.id ? "text-white" : "text-[#0D4715]"}>
                  {service.tabIcon}
                </span>
                {service.title}
              </button>
                    ...
                    {activeTab.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center gap-3 text-slate-700 font-semibold">
                        <div className="bg-blue-100 p-1 rounded-full">
                          <Check className="w-4 h-4 text-[#0D4715]" />
                        </div>
                        {feature}
                      </div>
                    ))}
